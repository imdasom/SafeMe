		...
		try {
			while (!Thread.interrupted()) {
				...
				if (length>0) {
					...
					//safeme
					int bufferSize = (rtphl+4) + length;
					ByteBuffer outputBuffer = ByteBuffer.allocate(bufferSize);
					outputBuffer.put(buffer, rtphl + 4, length);
					if(myMuxing!=null) {
						if (myMuxing.isStartMuxing()) {
							BufferInfo bufferInfo2 = bufferInfo;
							bufferInfo2.presentationTimeUs = System.nanoTime()/1000;
							myMuxing.getMediaMuxer().writeSampleData(myMuxing.getAudioTrackIndex(), outputBuffer, bufferInfo2);
							Log.d("safeme", myMuxing.getMediaMuxer()+"]] 오디오 먹싱중... "+bufferInfo2.offset+", "+bufferInfo2.size+", "+bufferInfo2.presentationTimeUs+", "+bufferInfo2.flags);
						}
					}
					outputBuffer.clear();
					//end of safeme
