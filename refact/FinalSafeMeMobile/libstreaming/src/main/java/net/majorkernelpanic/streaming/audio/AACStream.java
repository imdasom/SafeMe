...
public class AACStream extends AudioStream {
	...
	protected void encodeWithMediaCodec() throws IOException {
		...
		mAudioRecord = new AudioRecord(MediaRecorder.AudioSource.CAMCORDER, mQuality.samplingRate, AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT, bufferSize);
		...
		for(int i=0; i<samplingFreq.length; i++){
			if(samplingFreq[i]==mQuality.samplingRate){
				Log.d("safeme", "kSamplingFreq "+samplingFreq[i]+" i : "+i);
				sampleIndex = i;
			}
		}
		...
		if(sampleIndex == -1)
			Log.d("safeme", "kSamplingFreq = -1 ");
		else{
			csd = ByteBuffer.allocate(2);
			csd.put((byte) ((MediaCodecInfo.CodecProfileLevel.AACObjectLC<<3) | (sampleIndex >> 1)));
			csd.position(1);
			csd.put((byte) ((byte) (sampleIndex << 7) & 0x80 | (mChannel << 3)));
			csd.flip();
			...
			public void run() {
				...
				try {
					while (!Thread.interrupted()) {
						...
						//Log.d("safeme", "오디오에서 사용하는 MediaCodec bufferIndex : "+mMediaCodec+", "+bufferIndex);
						if (bufferIndex>=0) {
		...
		myMuxing.addAudioTrack(format);
		myMuxing.startMediaMuxer();

		mThread.start();

		// The packetizer encapsulates this stream in an RTP stream and send it over the network
		mPacketizer.setInputStream(inputStream);
		mPacketizer.setMediaMuxer(myMuxing);
		mPacketizer.start();
	...
	public synchronized void stop() {
		if (mStreaming) {
			...
			Log.d("safeme", "오디오 먹싱 중지");
			if(myMuxing.getMediaMuxer()!=null){	//safeme
				Log.d("safeme", "전체 먹싱 중지");
