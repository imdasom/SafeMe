...
public abstract class VideoStream extends MediaStream {
	...
	public synchronized void stop() {
		if (mCamera != null) {
			...
			//safeme
			Log.d("safeme", "비디오 먹싱 중지");
			if(myMuxing.getMediaMuxer()!=null)
	...
	protected void encodeWithMediaCodecMethod1() throws RuntimeException, IOException {
		...
		ByteBuffer sps = ByteBuffer.allocate(8);
		ByteBuffer pps = ByteBuffer.allocate(4);
		sps.put(myMuxing.getSPS());
		pps.put(myMuxing.getPPS());
		...
		Camera.PreviewCallback callback = new Camera.PreviewCallback() {
			...
			public void onPreviewFrame(byte[] data, Camera camera) {
				// 10sec => 10072116
				if((System.nanoTime()/1000 - myMuxing.timeStamp)>(10072116*6)){
					if(myMuxing.isStartMuxing()){
						Message msg = new Message();
						msg.what = 41; // safeme Constants.RECORD_FINISH;
						mHandler.sendMessage(msg);
						Log.d("safeme", "send Stop muxing");
					}
				}
				...
				try {
					...
					//Log.d("safeme", "비디오에서 사용하는 MediaCodec bufferIndex : "+mMediaCodec+", "+bufferIndex);
					if (bufferIndex>=0) {
		...
		// The packetizer encapsulates the bit stream in an RTP stream and send it over the network
		MediaCodecInputStream mediaCodecInputStream = new MediaCodecInputStream(mMediaCodec);	//safeme
		mediaCodecInputStream.setMediaMuxer(myMuxing);		//safeme

		mPacketizer.setInputStream(mediaCodecInputStream);		//safeme
		mPacketizer.start();
		myMuxing.timeStamp = System.nanoTime()/1000;

		//myMuxing.addVideoTrack(mediaFormat);		//safeme
